...
class Tank_auth
{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							return array(TRUE, $user->id, $user->username, 
                           ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED, $user->displayname);
	...
	function get_username()
	{
        ...
        /**
	 * Get the list of country name
	 * @return	array
	 */
        function get_countries()
        {
            return $this->ci->users->get_country_list();
        }
        /**
	 * Get the list of country name
	 * @return	array
	 */
        ...
        {
            return $this->ci->users->get_state_codes($country);
        }
	 ...
	 */
        function get_phone_number($user_id)
        {
            ...
            return $this->ci->users->get_phone_number($user_id);
        }
	...
	function create_user($username, $displayname, $email, $password, $phone, $email_activation, $country)
	{
		if((strlen($displayname) > 0)
		...
		} /*elseif (!$this->ci->users->is_phone_available($phone)) {
			$this->error = array('phone' => 'phone_in_use');
                }*/ else {
			// Hash password using phpass
                        $country_id = $this->ci->users->get_country_id($country);
			$hasher = new PasswordHash(
					$this->ci->config->item('phpass_hash_strength', 'tank_auth'),
					$this->ci->config->item('phpass_hash_portable', 'tank_auth'));
			...
			if (!is_null($res = $this->ci->users->create_user(
                                $data, !$email_activation))) 
                        {
	...
	{
		return ((strlen($phone) > 0) AND $this->ci->users->is_phone_available($phone));
	}
	/**
	 ...
	 * @return	bool
	 */
	function change_password($old_pass, $new_pass, $user_id)
	{
		if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
