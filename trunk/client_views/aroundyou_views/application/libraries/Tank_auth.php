...
class Tank_auth
{
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							return array(TRUE, $user->id, $user->username, 
                                                                ($user->activated == 1) ? STATUS_ACTIVATED : STATUS_NOT_ACTIVATED);
						}
					}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $phone, $email_activation)
	{
		...
		} elseif (!$this->ci->users->is_email_available($phone)) {
